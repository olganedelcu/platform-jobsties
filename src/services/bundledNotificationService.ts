...
import { AmazonSESNotificationService } from './amazonSESNotificationService';
...
class BundledNotificationManager {
  ...
  private readonly BUNDLE_DELAY = 30 * 1000; // 30 seconds for faster testing
  private sesEnabled: boolean = true; // Amazon SES is now the default

  constructor() {
    console.log("🔧 Amazon SES notification service initialized");
  }

  addNotification(notification: PendingNotification) {
    if (!this.sesEnabled) {
      console.warn("⚠️ Amazon SES not enabled, skipping notification bundling");
      return;
    }
  ...
  private async sendBundledNotifications(menteeId: string) {
    ...
    try {
      console.log(`📤 Sending ${notifications.length} bundled notifications to ${menteeInfo.menteeName} (${menteeInfo.menteeEmail}) via Amazon SES`);
      
      await AmazonSESNotificationService.sendBundledNotifications(
      ...
      console.log(`✅ Successfully sent bundled notifications to ${menteeInfo.menteeName} via Amazon SES`);
    } catch (error) {
      console.error(`❌ Failed to send bundled notifications to ${menteeInfo.menteeName} via Amazon SES:`, error);
      // Retry once after 5 seconds
      setTimeout(() => {
        console.log(`🔄 Retrying Amazon SES notification send for ${menteeInfo.menteeName}`);
        this.sendBundledNotifications(menteeId);
      }, 5000);
    } finally {
      // Clean up
  ...
  async flushNotifications(menteeId?: string) {
    console.log("🚀 Flushing notifications immediately via Amazon SES...");
    if (menteeId) {
      await this.sendBundledNotifications(menteeId);
    } else {
      // Send all pending notifications
      const menteeIds = Array.from(this.pendingNotifications.keys());
      console.log(`📤 Flushing notifications for ${menteeIds.length} mentees via Amazon SES`);
  ...
  getStatus() {
    ...
    console.log("📊 Amazon SES notification service status:", status);
    return status;
  }
}
...
export const BundledNotificationService = {
  configure(endpoint?: string) {
    // No longer needed for Amazon SES, but keeping for backward compatibility
    console.log("🔧 Amazon SES is now the primary email service - no endpoint configuration needed");
  },

  addJobRecommendation(menteeId: string, menteeEmail: string, menteeName: string, jobTitle: string, companyName: string) {
    console.log("➕ Adding job recommendation to Amazon SES notification bundle");
  ...
  addFileUpload(menteeId: string, menteeEmail: string, menteeName: string, fileName: string) {
    console.log("➕ Adding file upload to Amazon SES notification bundle");
    bundledNotificationManager.addNotification({
      id: `file_${Date.now()}`,
      menteeId,
      menteeEmail,
  ...
  addMessage(menteeId: string, menteeEmail: string, menteeName: string, messagePreview: string) {
    console.log("➕ Adding message to Amazon SES notification bundle");
    bundledNotificationManager.addNotification({
  ...
  addTodoAssignment(menteeId: string, menteeEmail: string, menteeName: string, todoTitle?: string, count?: number) {
    console.log("➕ Adding todo assignment to Amazon SES notification bundle");
    bundledNotificationManager.addNotification({
      id: `todo_${Date.now()}`,
      menteeId,
      menteeEmail,
