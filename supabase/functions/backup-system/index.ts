...
async function performFullDatabaseBackup(supabase: any, compress: boolean) {
  ...
  // Upload to storage as JSON file with proper content type
  const { error: uploadError } = await supabase.storage
    .from('backups')
    .upload(
      filePath, 
      new File([backupContent], fileName, { type: 'application/json' }),
      {
...
async function performStorageBackup(supabase: any, compress: boolean) {
  ...
  const { error: uploadError } = await supabase.storage
    ...
    .upload(
      filePath, 
      new File([manifestContent], fileName, { type: 'application/json' }),
      {
        contentType: 'application/json',
        upsert: false
      }
